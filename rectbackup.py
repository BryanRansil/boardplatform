...
class ShapeHandler(webapp.RequestHandler):
	...
	#parse this!
	#1st argument will be the action, so we can parse through to the second equal sign.
	length = len(body)
	start = len("action=DrawRect&")
	rectargs = [0, 0, 0, 0, 0]
	i = start 
	while (body[i] != '=') :
            i = i+1 
	...
	while (body[i] != '=') :
        ...
        rectargs[3] = body[beginning:end]
        result = self.methods.DrawRect(*rectargs)
